...
import { map, repeat, zipWith, concat } from "ramda";
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, values } from './L5-ast';
...
import { LetValuesExp,valBinding, isLetValuesExp, isValuesExp} from "./L5-ast"
import { isNonEmptyTupleTExp,isNumTExp,isBoolTExp,isStrTExp,isVoidTExp,isEmptyTVar,isProcTExp,isTVar, NonTupleTExp, ProcTExp, TVar} from "./TExp";
...
//(L5 (let-values  ((( x y) (values 1 2)))(+ x y)) )

// Evaluate a sequence of expressions (in a program)
export const evalSequence = (seq: Exp[], env: Env): Result<Value> =>{
 return  isEmpty(seq) ? makeFailure("Empty sequence") :
...
const evalCExps = (first: Exp, rest: Exp[], env: Env): Result<Value> =>{
    //console.log(first);
    //console.log(rest);
    if(isCExp(first) && isEmpty(rest)){
        return applicativeEval(first, env);
    }
    else if(isCExp(first)){
        const test = bind(applicativeEval(first, env), _ => evalSequence(rest, env));
        console.log(test);
        return test;
    }
    //return isCExp(first) && isEmpty(rest) ? applicativeEval(first, env) :
    //isCExp(first) ? bind(applicativeEval(first, env), _ => evalSequence(rest, env)) :
    return makeFailure("NeverevalCExps");
}
...
// LETValues: Direct evaluation rule without syntax expansion
// compute the values, extend the env, eval the body.
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> =>{
    const vals =mapResult((v : CExp[]) => help1(v, env), map((b : valBinding) => b.val, exp.bindings));
    const vars = map((b: valBinding) => help2(b.var), exp.bindings);
    const temp1 = vars.reduce(concat);
    if(isOk(vals)){
        const temp= vals.value.reduce(concat);
        const temp3 = makeOk(temp);
        return bind(temp3, (vals: Value[]) => evalSequence(exp.body, makeExtEnv(temp1, vals, env)));
    }
    return makeFailure("Error-evalLetValues!");   
}

const help1 = (exps: CExp[] ,env: Env) : Result<SExpValue[]> =>
 mapResult ((v : CExp) => applicativeEval(v, env) , exps);

const help2 = (exps: VarDecl[] ) : string[] =>
      map( (v: VarDecl) => v.var, exps);

//values: Direct evaluation rule without syntax expansion
//values {tag: "values"; var:CExp[];}
const evalValues = (value: values, env: Env): Result<Value> =>
      evalSequence(value.var, env) ;
/*
{
    console.log("enter evalValues");
    console.log(value.var);
    //return  isCExp(first) ? bind(applicativeEval(first, env), _ => evalSequence(rest, env)) :
     return deme(first(value.var),rest(value.var), env);    
}
*/

const deme = (first: Exp, rest: Exp[], env: Env): Result<Value> =>{
    //console.log(first);
    //console.log(rest);
    if(isCExp(first) && isEmpty(rest)){
        return applicativeEval(first, env);
...
/*
export const evalSequenceTuple = (seq: NonTupleTExp[], env: Env): Result<Value> =>
    isEmpty(seq) ? makeFailure("Empty sequence") :
    evalExpsTuple(first(seq), rest(seq), env);
 
const evalExpsTuple = (first: NonTupleTExp, rest: NonTupleTExp[], env: Env): Result<Value> =>
    isEmpty(rest) ? applicativeEvalTuple(first, env) :
    bind(applicativeEvalTuple(first, env), _ => evalSequenceTuple(rest, env));

 export const applicativeEvalTuple = (exp: NonTupleTExp, env: Env): Result<Value> =>
 isNumTExp(exp) ? makeOk(exp.tag) :
 isBoolTExp(exp) ? makeOk(exp.tag) :
 isStrTExp(exp) ? makeOk(exp.tag) :
 isVoidTExp(exp) ? makeOk(exp.tag) :
 isEmptyTVar(exp) ? makeOk(exp.tag) :
 //isTVar(exp) ? evalTVarTexp(exp,env) :
 //isProcTExp(exp) ?evalProcTexp(exp,env) :
 makeFailure("NeverapplicativeEvalTuple");  
*/
 /*
 const evalTVarTexp = (exp: TVar, env: Env): Result<Value> => {

}

const evalProcTexp = (exp: ProcTExp, env: Env): Result<Value> => {
