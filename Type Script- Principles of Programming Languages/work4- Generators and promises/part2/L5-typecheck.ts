// L5-typecheck
// ========================================================
import { equals, map, zipWith, concat } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, LetValuesExp, values, CExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTupleTExp, TupleTExp, isNonEmptyTupleTExp, NonTupleTExp, isNumTExp, isBoolTExp, isStrTExp, isVoidTExp, isEmptyTVar, isTVar, isEmptyTupleTExp, makeNonEmptyTupleTExp, SecondParseTExp, makeEmptyTupleTExp, ProcTExp, makeTVar } from "./TExp";
...
export const typeofExp = (exp: Parsed | NonTupleTExp | TupleTExp, tenv: TEnv): Result<TExp> =>
    ...
    isTupleTExp(exp) ? typeofTuple(exp, tenv) :
    isNumTExp(exp) ? makeOk(exp) :
    isBoolTExp(exp) ? makeOk(exp) :
    isStrTExp(exp) ? makeOk(exp) :
    isVoidTExp(exp) ? makeOk(exp) :
    isEmptyTVar(exp) ? makeOk(exp) :
    isTVar(exp) ? makeOk(exp) :         //typeofTVarTexp(exp,tenv) :??
    isProcTExp(exp) ?  makeOk(exp) :    //typeofProcTexp(exp,tenv) :??
...
export const typeofNonTuple = (exps: NonTupleTExp[], tenv: TEnv): Result<TExp> =>
isEmpty(rest(exps)) ? typeofExp(first(exps), tenv) :
bind(typeofExp(first(exps), tenv), _ => typeofNonTuple(rest(exps), tenv));

const typeofTuple=(t:TupleTExp, tenv: TEnv):Result<TExp>=>{
    return isNonEmptyTupleTExp(t) ? typeofNonTuple(t.TEs, tenv):
...
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const varsTEs = map((vd) => vd.var, exp.bindings);
    const varsArray = varsTEs.reduce(concat);
    const varsString = map((b) => b.var,varsArray);
    const Texps = map((b) => b.texp ,varsArray);
    const expTEs = map((b) => b.val,  exp.bindings);
    const expArray = expTEs.reduce(concat);
    const constraints = zipWithResult((varTE, val) => bind(typeofExp(val, tenv),
                                                           (typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)),
                                                           Texps, expArray);
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(varsString, Texps, tenv)));
};

export const typeofValues=(exp: values, tenv: TEnv): Result<TExp> => {
    if(isEmpty(exp.var)){
        return makeFailure("error? empty!"); 
    } 
    const t= mapResult((arg) => ParseNon(arg, tenv), exp.var);
    if(isOk(t)){
        return makeOk(makeNonEmptyTupleTExp(t.value));  
    }
    return makeFailure("its not ok"); 
}
//change
export const ParseNon = (texp: CExp, tenv: TEnv): Result<NonTupleTExp> =>
isNumExp(texp)? makeOk(makeNumTExp()):
isStrExp(texp)? makeOk(makeStrTExp()):
isBoolExp(texp)? makeOk(makeBoolTExp()):
isProcExp(texp)? typeofProc2(texp,tenv):
makeOk(makeVoidTExp()) ;  

export const typeofProc2 = (proc: ProcExp, tenv: TEnv): Result<ProcTExp> => {
    const argsTEs = map((vd) => vd.texp, proc.args);
    const extTEnv = makeExtendTEnv(map((vd) => vd.var, proc.args), argsTEs, tenv);
    const constraint1 = bind(typeofExps(proc.body, extTEnv),
                             (body: TExp) => checkEqualType(body, proc.returnTE, proc));
