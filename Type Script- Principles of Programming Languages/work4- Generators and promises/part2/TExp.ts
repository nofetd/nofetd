...
import { isEmpty, cons } from "../shared/list";
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    (texp === "Empty") ? makeOk(makeEmptyTupleTExp()) :
...
export const SecondParseTExp = (texp: Sexp): Result<NonTupleTExp> =>
    (texp === "number") ? makeOk(makeNumTExp()) :
    (texp === "boolean") ? makeOk(makeBoolTExp()) :
    (texp === "void") ? makeOk(makeVoidTExp()) :
    (texp === "string") ? makeOk(makeStrTExp()) :
    isString(texp) ? makeOk(makeTVar(texp)) :
    isArray(texp) ? SecondparseCompoundTExp(texp) :
    makeFailure(`Unexpected TExp - ${texp}`);

const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    const pos = texps.indexOf('->');
    return (pos === -1)  ? parseTuple(texps):  //makeFailure(`Procedure type expression without -> - ${texps}`) :
           ...
           safe2((args: TExp[], returnTE: TExp) => makeOk(makeProcTExp(args, returnTE)))
                (parseTupleTExp(texps.slice(0, pos)), SecondParseTExp(texps[pos + 1]));
};

const parseTuple = (texps: Sexp[]): Result<TupleTExp> =>{ 
    ...
    const pos1 = texps.indexOf('Empty');
    if(pos1 >-1){
        return makeOk(makeEmptyTupleTExp());
    }
    const pos2 = texps.indexOf('*');
    if(pos2>-1){
        const arr = splitEvenOdds(texps);
        if(isOk(arr)){
           return bind ( mapResult(SecondParseTExp,arr.value), (args: NonTupleTExp[]) => makeOk(makeNonEmptyTupleTExp(args)));
        }
    }
    return bind ( mapResult(SecondParseTExp,texps), (args: NonTupleTExp[]) => makeOk(makeNonEmptyTupleTExp(args)));
     
}

export const SecondparseCompoundTExp = (texps: Sexp[]): Result<NonTupleTExp> => {
    const pos = texps.indexOf('->');
    return (pos === -1)  ?makeFailure(`Procedure type expression without -> - ${texps}`) :
           ...
           (texps.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`Only one -> allowed in a procexp - ${texps}`) :
           safe2((args: TExp[], returnTE: TExp) => makeOk(makeProcTExp(args, returnTE)))
                (parseTupleTExp2(texps.slice(0, pos)), SecondParseTExp(texps[pos + 1]));
};

const parseTupleTExp2 = (texps: Sexp[]): Result<NonTupleTExp[]> => {
    ...
    return isEmptyTuple(texps) ? makeOk([]) : bind(splitEvenOdds(texps),
                                                       (argTEs: Sexp[]) => mapResult(SecondParseTExp, argTEs));
    }
...
const unPaarsetup=(tuple:TupleTExp): Result <string>=>{
    if(isEmptyTupleTExp(tuple)){
        return makeOk("(Empty)");
    }
    const str = mapResult(unparseTExp,tuple.TEs);
    if(isOk(str)){
        return makeOk(`(${str.value.join(" * ")})`);
    }
    return makeFailure("error in unPaarsetup!");
}
