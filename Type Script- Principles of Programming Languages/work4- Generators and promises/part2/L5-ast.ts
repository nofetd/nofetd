...
import { Value, isCompoundSExp, isEmptySExp, isSymbolSExp, makeCompoundSExp, makeEmptySExp, makeSymbolSExp, SExpValue, valueToString } from './L5-value';
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp,makeNumTExp,makeBoolTExp,makeVoidTExp,makeStrTExp,makeTVar,SecondparseCompoundTExp, makeProcTExp} from './TExp';
...
import { isNonEmptyTupleTExp, isEmptyTupleTExp, NonTupleTExp, TupleTExp, makeEmptyTupleTExp, makeNonEmptyTupleTExp } from "./TExp";
...
export type CExp =  AtomicExp | CompoundExp | values;
export const isCExp = (x: any): x is CExp => isAtomicExp(x) || isCompoundExp(x) || isValuesExp(x);
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | LetValuesExp | SetExp;
...
export interface values {tag: "values"; var: CExp[]; }
export const makeValues = (v: CExp[]): values =>
    ({tag: "values", var: v});
...
export interface valBinding {tag: "ValBinding"; var: VarDecl[]; val: CExp[]; }
export const makevalBinding = (v: VarDecl[], val: CExp[]): valBinding =>
    ({tag: "ValBinding", var: v, val: val});
export const isValBinding = (x: any): x is valBinding => x.tag === "ValBinding";
...
export interface LetValuesExp {tag: "LetValuesExp"; bindings: valBinding[]; body: CExp[]; }
export const makeLetValuesExp = (bindings: valBinding[], body: CExp[]): LetValuesExp =>
...
export const parseL5Exp = (sexp: Sexp): Result<Exp> =>{
    console.log(sexp);
    return isEmpty(sexp) ? makeFailure("Exp cannot be an empty list") :
...
export const parseL5SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>{
    ...
    return isEmpty(params) ? makeFailure("Empty args for special form") :
...
// values -  1 (lambda (x) x x) #t)  
// values - NonEmptyTuple -  TVar, Pro 
export const parseValues = (params: Sexp[]): Result<CExp> =>
    isEmpty(params) ?  makeFailure("Empty args for values") :
    bind(mapResult(parseL5CExp, params), (args: CExp[]) => makeOk(makeValues(args)));

    /*
const parseToTuple=(params: Sexp): Result<NonTupleTExp>=>
    (params === "number") ? makeOk(makeNumTExp()) :
    (params === "boolean") ? makeOk(makeBoolTExp()) :
    (params === "void") ? makeOk(makeVoidTExp()) :
    (params === "string") ? makeOk(makeStrTExp()) :  
    isString(params) ? makeOk(makeTVar(params)) :
   // isArray(params) ? makecompounNonn(first(params), rest(params)) :
   //isArray(params) ? parseL5CompoundCExp(first(params), rest(params)) :
    makeFailure(`Unexpected TExp - ${params}`);
/*    
const makecompounNonn=(op: Sexp, params: Sexp[]): Result<NonTupleTExp>=>
//op === "if" ? parseIfExp(params) :
op === "lambda" ?  parseProcExp(first(params), rest(params)) :
op === "let" ? parseLetExp(first(params), rest(params)) :
op === "let-values" ? parseLetValuesExp(first(params), rest(params)):
...
/*
export const parseValues = (params: Sexp[]): Result<values> =>
    isEmpty(params) ? makeOk(makeValues(makeEmptyTupleTExp())) :
    bind(mapResult(parseToTuple, params), (args: NonTupleTExp[]) => makeOk(makeValues(makeNonEmptyTupleTExp(args))));
...
const parseToTuple=(params: Sexp): Result<NonTupleTExp>=>
    ...
    (params === "string") ? makeOk(makeStrTExp()) :
    isString(params) ? makeOk(makeTVar(params)) :
    isArray(params) ? SecondparseCompoundTExp(params) :
    makeFailure(`Unexpected TExp - ${params}`);
*/
...
export const parseL5CExp = (sexp: Sexp): Result<CExp> =>{
    //console.log("insert to func");
    //console.log(sexp);
    return isEmpty(sexp) ? makeFailure("CExp cannot be an empty list") :
...
const isSpecialForm = (x: string): boolean =>
    ["if", "lambda", "let", "let-values", "quote", "letrec", "set!", "values"].includes(x);
...
const isGoodValBindings=(bindings: Sexp): bindings is [Sexp[], Sexp[]][]=>
isArray(bindings) && allT(isArray, bindings);
...
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> => 
    isEmpty(body) ? makeFailure('Body of "let" cannot be empty') :
    ! isGoodValBindings(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`) :
    safe2((bdgs: valBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
        (parseValBindings(bindings), mapResult(parseL5CExp, body));
...
const parseValBindings = (bindings: [Sexp[], Sexp[]][]): Result<valBinding[]> =>
safe2((vds: VarDecl[][], vals: CExp[][]) => makeOk(zipWith(makevalBinding, vds, vals)))
        (mapResult(lol1, map(b => b[0], bindings)), mapResult(lol2, map(b => b[1], bindings)));

const lol1 = (b: Sexp[]): Result<VarDecl[]> =>
mapResult(parseVarDecl, b);

const lol2 = (b: Sexp[]): Result<CExp[]> =>
mapResult(parseL5CExp, b); 
...
const unparseValues =(v: values): Result<string> =>{
    const str = mapResult(unparse, v.var);
    if(isOk(str))
    {
        const x = str.value.join(" ");
        return makeOk(`(values ${x})`);
    }
...
const help1 = (b: VarDecl[]): Result<string[]> =>
    mapResult(unparseVarDecl, b);

const help2 = (b: CExp[]): Result<string[]> =>
    mapResult(unparse, b);

const unparseValBindings = (bindings: valBinding[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string[], val: string[]) => makeOk(`((${vd.join(" ")}) (${val.join(" ")}))`))(help1(bdg.var), help2(bdg.val)), bindings),
         (bdgs: string[]) => makeOk(join(" ", bdgs)));
...
const unparseLetValueExp = (le: LetValuesExp) : Result<string> => 
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseValBindings(le.bindings), unparseLExps(le.body));
