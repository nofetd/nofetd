...
(define get-lst$
  (lambda (assoc-list cont)
   (cont (cdr (car assoc-list)))))   
...
(define collect-all-values-1
 (lambda (lists key)
   (cond ((eq? lists '()) '())
         ((eq? (get-value (car lists) key)'fail) (collect-all-values-1 (cdr lists) key))
...
(define collect-all-values-2
 (lambda (lists key)
   (cond ((eq? lists '()) '())
         ((eq? (get-value$ (car lists) key (lambda(x) x) (lambda()#f)) #f) (collect-all-values-2 (cdr lists) key))
         (else(cons (get-value$ (car lists) key (lambda(x) x) (lambda()#f)) (collect-all-values-2 (cdr lists) key))))
 )
) 
