...
export const makeForExp = (loop: VarDecl, start: CExp, end: CExp, body: CExp): ForExp =>
...
// <ForExp> -> (for <VarDecl> <NumExp> <NumExp> <CExp>)
const parseForExp = (vars: Sexp,params: Sexp[]): Result<ForExp> =>
    params.length !== 3 ? makeFailure("Expression is not of the form (for <VarDecl> <NumExp> <NumExp> <CExp>)") :
    isVarDecl(vars) && isNumExp(params[0]) && isNumExp(params[1]) && isCExp(params[2]) ?  
        bind( mapResult(parseL21CExp, params),
             (cexps: CExp[]) => makeOk(makeForExp(vars, cexps[0] , cexps[1], cexps[2]))) :
    makeFailure("Invalid vars for ForExp");
